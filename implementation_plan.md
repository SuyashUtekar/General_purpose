...
```mermaid
graph TD
    A[User Query] --> B[L1 Classifier Node]
    B -- Valid --> C[Decompose & Paraphrase Node]
    B -- Invalid --> D[Error Handler Node]
    C --> E[L2 Classifier Node]
    E --> F[Planner Agent]
    F --> G{Route Sources}
    G -- SQL --> H[NL-SQL Tool]
...
- **[l1_classifier.py](file:///c:/Users/admin/OneDrive/Documents/DRISHTI/drishti/nodes/l1_classifier.py)**: Performs regex sanitization (SQL Injection/XSS), length/char check, domain keyword scanner, and lightweight LLM validation.
- **[decomposition.py](file:///c:/Users/admin/OneDrive/Documents/DRISHTI/drishti/nodes/decomposition.py)**: Performs SpaCy-based NER and dependency parsing, and flan-t5-small/Gemini-based query paraphrasing in parallel.
- **[l2_classifier.py](file:///c:/Users/admin/OneDrive/Documents/DRISHTI/drishti/nodes/l2_classifier.py)**: Signal-based query router checking keyword lists before falling back to a structured LLM call.
